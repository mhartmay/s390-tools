...
#define STAGE3A_ARGS(data_ptr, loader_size) \
	((struct stage3a_args *)(data_ptr + loader_size - sizeof(struct stage3a_args)))
#define STAGE3A_DATA(data_ptr, loader_size, offset) \
	((void *)(data_ptr + PAGE_ALIGN(loader_size) + offset))

static Buffer *loader_getblob(const char *filename, gsize *loader_size, size_t args_size,
			      size_t data_size, gboolean data_aligned, GError **err)
{
	g_autoptr(Buffer) ret = NULL;
	g_autoptr(GMappedFile) mapped_file = NULL;
	gchar *loader_data;
	gsize tmp_loader_size;
	size_t size;
	...
	if (!loader_data) {
		g_set_error(err, G_FILE_ERROR, G_FILE_ERROR_BADF, _("File %s is empty"), filename);
		return NULL;
	}
	...
	if (tmp_loader_size < args_size) {
		g_set_error(err, G_FILE_ERROR, G_FILE_ERROR_BADF,
			    _("File size less than expected: %lu < %ln"), tmp_loader_size,
			    loader_size);
		return NULL;
	}
	 ...
	 * aligned. */
	size = (data_aligned ? PAGE_ALIGN(tmp_loader_size) : tmp_loader_size) + data_size;
	...
	memset(ret->data + tmp_loader_size - args_size, 0, args_size);
	*loader_size = tmp_loader_size;
	return g_steal_pointer(&ret);
}

Buffer *stage3a_getblob(const char *filename, gsize *loader_size, gsize data_size, GError **err)
{
	return loader_getblob(filename, loader_size, sizeof(struct stage3a_args), data_size, TRUE,
			      err);
}
...
static int stage3a_set_data(Buffer *loader, gsize loader_size, const Buffer *hdr,
			    struct ipl_parameter_block *ipib, GError **err)
{
	...
	size_t args_size = sizeof(struct stage3a_args);
	uint64_t args_addr, next_data_addr;

	if (hdr->size > UINT32_MAX) {
		g_set_error(err, PV_ERROR, PV_ERROR_INTERNAL, _("Invalid header size: %zu"),
			    hdr->size);
		return -1;
	}

	/* we assume here that the loader ``stage3a`` will be 4k page
	 * aligned loaded in the guest */
	args_addr = (uint64_t)loader->data + loader_size - args_size;

	/* therefore `next_data_addr` is also 4k page aligned */
...
int build_stage3a(Buffer *loader, gsize loader_size, const Buffer *hdr,
		  struct ipl_parameter_block *ipib, GError **err)
{
	int rc = stage3a_set_data(loader, loader_size, hdr, ipib, err);
	if (rc)
		return -1;
	return 0;
}

Buffer *stage3b_getblob(const char *filename, GError **err)
{
	g_autoptr(Buffer) ret = NULL;
	gsize rb_size;

	ret = loader_getblob(filename, &rb_size, sizeof(struct stage3b_args), 0, FALSE, err);
	if (!ret)
		return ret;
...
void build_stage3b(Buffer *stage3b, const struct stage3b_args *args)
{
	 ...
	 * positioned */
	memcpy(stage3b->data + stage3b->size - sizeof(*args), args, sizeof(*args));
}
