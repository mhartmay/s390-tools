...
uint64_t pv_ipib_get_size(uint32_t num_comp)
{
	...
	return MAX(sizeof(struct ipl_pl_hdr) + sizeof(struct ipl_pb0_pv) +
			   num_comp * sizeof(struct ipl_pb0_pv_comp),
		   PAGE_SIZE);
}

static int pv_ipib_init(IplParameterBlock *ipib, GSList *comps, const Buffer *hdr)
{
	...
	g_assert_true(g_uint_checked_mul(&ipib_comps_size, comps_length,
					 (uint32_t)sizeof(struct ipl_pb0_pv_comp)));
	g_assert_true(g_uint_checked_add(&blk0_len, (uint32_t)sizeof(*pv), ipib_comps_size));
	...
	for (GSList *iterator = comps; iterator; iterator = iterator->next, i++) {
		...
		g_assert(comp);
		uint64_t comp_addr = pv_component_get_src_addr(comp);
		uint64_t comp_size = pv_component_size(comp);
		...
		pv->components[i].tweak_pref = GUINT64_TO_BE(pv_component_get_tweak_prefix(comp));
		if (i > 0) {
...
IplParameterBlock *pv_ipib_new(GSList *comps, const Buffer *hdr, GError **err)
{
	...
	if (ipib_size > PAGE_SIZE) {
		g_set_error(err, PV_ERROR, PV_ERROR_IPIB_SIZE, _("IPIB size too large: %lu < %lu"),
			    ipib_size, PAGE_SIZE);
		return NULL;
	}

	ret = g_malloc0(ipib_size);
	rc = pv_ipib_init(ret, comps, hdr);
	if (rc < 0)
		return NULL;
