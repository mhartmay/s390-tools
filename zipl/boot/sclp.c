...
int sclp_print_ascii(const char *buffer)
{
	...
	sccb = (void *)get_zeroed_page();
	sccb->header.length = sizeof(struct write_sccb) - sizeof(struct mdb) + data_len;
	sccb->header.function_code = SCLP_FC_NORMAL_WRITE;
	sccb->msg_buf.header.length = sizeof(struct msg_buf) - sizeof(struct mdb) + data_len;
	sccb->msg_buf.header.type = SCLP_EVENT_DATA_ASCII;
	sccb->msg_buf.header.flags = 0;
	ebcdic_to_ascii(sccb->msg_buf.data, (const unsigned char *)buffer, data_len - 1);
